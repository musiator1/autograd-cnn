...
- zamiast alokować pamięć na wycinek tablicy (tab[idx1:idx2]) stosuje się widok, czyli okno (wskaźnik) na te pożądane dane
- zmiany w kodzie: primal! oraz adjont! dla im2col oraz maxpool
=#
...
function adjoint!(z::GraphNode{:mul})
    W, x = z.args
    mul!(W.grad, z.grad, x.data', 1.0, 1.0)
    mul!(x.grad, W.data', z.grad, 1.0, 1.0)       
    return nothing
end
...
function primal!(z::GraphNode{:maxPool})
    ...
    H_w = div(H_in, H_out)
    W_w = div(W_in, W_out)
    @views for c in 1:C, j in 1:W_out, i in 1:H_out
        h_start = (i-1)*H_w + 1
        w_start = (j-1)*W_w + 1
