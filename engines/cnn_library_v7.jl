#=
OPTYMALIZACJE:
- usunięcie slicing przy pętlach dla im2col oraz maxpool, co sprawia że nie ma potrzeby robić widoków oraz stosować funkcji vec()
- zamiast slicing jest stosowana większa ilość pętli
=#
...
function (y::Conv)(x)
    ...
    H_out = H_pad - H_k + 1
    W_out = W_pad - W_k + 1
    x_col = GraphNode(:im2col, (x_pad,), zeros(Float32, H_out * W_out, H_k * W_k * C_in))

    # Mul, resize
    mul = GraphNode(:mul, (x_col, W), zeros(Float32, H_out * W_out, C_out))
...
function primal!(z::GraphNode{:im2col})
    ...
    for j in 1:W_out, i in 1:H_out
        patch_idx = 1
        for c in 1:C, w in 1:W_k, h in 1:H_k
            z.data[idx, patch_idx] = X[i+h-1, j+w-1, c]
            patch_idx += 1
        end
        idx += 1
    end
...
function adjoint!(z::GraphNode{:im2col})
    ...
    for j in 1:W_out, i in 1:H_out
        patch_idx = 1
        for c in 1:C, w in 1:W_k, h in 1:H_k
            x.grad[i+h-1, j+w-1, c] += z.grad[idx, patch_idx]
            patch_idx += 1
        end
        idx += 1
    end
...
function primal!(z::GraphNode{:maxPool})
    ...
    for c in 1:C, j in 1:W_out, i in 1:H_out
        ...
        max_val = -Inf32
        for wj in 1:W_w, wi in 1:H_w
            value = x.data[h_start + wi, w_start + wj, c]
            if value > max_val
                max_val = value
            end
        end
        z.data[i, j, c] = max_val
    end
    return nothing
end
...
function adjoint!(z::GraphNode{:maxPool})
    ...
    for c in 1:C, j in 1:W_out, i in 1:H_out
        ...
        x.grad[max_h, max_w, c] += z.grad[i, j, c]
    end
    return nothing
end
